...
### 5. 使用第三方插件
```plaintext
注意：由于`Flandre`的管理系统与插件间存在一定程度的整合，使用第三方插件可能出现一些问题，例如无法使用服务管理器开关、命令冲突、存储冲突等，请谨慎使用。如有必要，建议在插件的协议允许的范围内手动进行一定的适配，适配方法可以参考内置插件
```
...
- [x] Bot管理
  ...
  - [x] 错误报告
  - [x] 运行状态获取
  - [x] 登入、登出提示（go-cqhttp经常接受不到这类事件，原因不明……）
  - [x] 反馈系统
