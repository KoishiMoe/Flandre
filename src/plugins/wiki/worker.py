...
wiki_article = on_regex(ARTICLE_RAW, permission=GROUP, state={"mode": "article"}, rule=online("wiki") & gag())
...
async def wiki_preprocess(bot: Bot, event: GroupMessageEvent, state: T_State, matcher: Matcher):
    if not await check_limit(bot, event, "wiki", False):
        await matcher.finish("啊啦，你查的有一点点多……也许是时候让该让维基娘休息一下了(．． )…")
